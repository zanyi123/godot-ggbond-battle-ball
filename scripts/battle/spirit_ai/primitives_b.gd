class_name SpiritAIPrimitivesB
extends RefCounted
## 波B原语：生存/控制族（27标签）+ 受击反应增强。契约见 元灵技能AI规划/06_原语接口规约.md
## 纪律：禁 randf/instance_id（骰子输入）/直连 ai_manager 感知数据（感知走 ctx）；缩进 Tab
## gate 判定与波A同构自包含（05§一 白名单分文件互斥设计，避免跨窗口类依赖）
## 波B增强：ctx 可选字段 "reaction_hot"（bool，缺省 false fail-closed）——
##   事件钩子层 event_hooks.gd 报告"该球员刚受击/被异常状态，处于反应热窗口"，
##   manager 接线方式见 event_hooks.gd 头注（本波只交付钩子层与增强逻辑，接线归集成窗口）

const TABLE_PATH := "res://data/systems/spirit_ai/primitives_b.json"
...
## self_threatened 的体力低判定阈值（06 §三：stamina_ratio < 0.4 视同受威胁）
...
static func _load_table(path: String) -> Dictionary:
	if not FileAccess.file_exists(path):
		push_error("SpiritAIPrimitivesB: 原语表不存在 " + path)
		return {}
	var text: String = FileAccess.get_file_as_string(path)
	if text.is_empty():
		push_error("SpiritAIPrimitivesB: 原语表为空 " + path)
		return {}
	var parsed = JSON.parse_string(text)
	if not parsed is Dictionary:
		push_error("SpiritAIPrimitivesB: 原语表 JSON 解析失败 " + path)
		return {}
	var raw: Dictionary = parsed
	var descs = raw.get("descriptors", {})
	if not descs is Dictionary:
		push_error("SpiritAIPrimitivesB: descriptors 字段缺失或非法 " + path)
		return {}
	return descs
...
##   可选："allies": Array（ally_injured 用）、"reaction_hot": bool（波B事件钩子热窗口）、
##         "ball_in_flight": bool（ball_flight，操球窗口启用）、
...
static func timing_gate(descriptor: Dictionary, sad: Dictionary, ctx: Dictionary) -> Dictionary:
	...
	match gate_key:
		...
		"self_threatened":
			# 波B增强：反应热窗口（刚受击/被控）视同受威胁——受击瞬间反应的闸门通道
			...
			if not threatened:
				return {"ok": false, "bonus": 1.0}
			# 两档 bonus：强触发（敌贴脸 <100px 或反应热窗口）给 1.25，其余 1.0
		...
		"ally_injured":
			...
			# 条目兼容两种格式：Dictionary{"stamina_ratio": float} 或带体力属性的球员对象
...
## value_param="none"（on_hit_expire/unroot 等无数值参数标签）恒回落 value_min（06 §2.2 合规用法）
